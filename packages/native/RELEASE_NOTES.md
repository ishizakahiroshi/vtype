...
| File | For |
|---|---|
| `vtype-0.1.0-windows-x64.zip` | Windows 10 / 11 (x64). Unzip it and double-click `vtype.exe`. The first-run screen asks whether to start it with Windows (ticked by default); `vtype install` does the same from the command line |
...
| `SHA256SUMS.txt` | Checksums of the files above, except `vtype.rb` (which carries the macOS checksum itself) |
